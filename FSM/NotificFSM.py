...
# @dp.message_handler(commands='Загрузить', state=None)
async def cms_start1(message: types.Message):
...
async def cms_start2(message: types.Message):
...
async def cms_start3(message: types.Message):
...
async def cms_start4(message: types.Message):
    # message.text Сохраняем в бд (Как узнали)
    await FSMsAdmin.first()
